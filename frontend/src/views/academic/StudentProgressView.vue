<template>
  <v-card>
    <v-card-text>
      ...
      >
        <template #actions>
          <v-btn variant="text" :loading="loading" @click="fetchProgress">Tải lại</v-btn>
        </template>
      </PageHeader>
          ...
          <v-col cols="6" md="4">
            <v-card class="h-100" color="success" variant="tonal" border>
              <v-card-text class="text-center d-flex flex-column justify-center h-100">
                ...
                <div class="text-h2 font-weight-bold mb-2">{{ progress.gpa10 }}</div>
          ...
          <v-col cols="6" md="4">
            <v-card class="h-100" color="info" variant="tonal" border>
              <v-card-text class="text-center d-flex flex-column justify-center h-100">
                ...
                <div class="text-h2 font-weight-bold mb-2">{{ progress.gpa4 }}</div>
                  ...
                  >
                    <v-expansion-panel-title>
                      <div class="d-flex align-center w-100">
                        <v-icon
                            :icon="sem === 'extra' ? 'mdi-plus-circle-outline' : 'mdi-calendar-check'"
                            :color="sem === 'extra' ? 'info' : 'primary'"
                            class="mr-3"
                        ></v-icon>
              ...
              <v-window-item value="all">
                ...
                >
                  ...
                  <template v-slot:item.finalScore="{ item }">
                    <span
                        v-if="item.finalScore !== null"
                        :class="{ 'font-weight-bold': true, 'text-success': item.status === 'PASSED', 'text-error': item.status === 'NOT_PASSED' }"
                    >
                      {{ item.finalScore }}
                    </span>
...
// Đọc backTo từ query param (khi navigate từ TeacherAdminClassStudentsView),
// fallback về trang mặc định nếu dùng standalone
const backTo = computed(() => (route.query.backTo as string) || '/teacher/admin-classes')
...
const sortedSemesters = computed(() => {
  ...
  if (groupedSubjects.value['extra']) result.push('extra')
  return result
})
...
const targetStudentId = computed(() => {
  return props.studentId ?? (route.params.studentId ? Number(route.params.studentId) : authStore.user?.studentId)
})
