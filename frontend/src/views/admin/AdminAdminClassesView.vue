...
const deleting = ref<AdminClass | null>(null)
...
const openEditDialog = (c: AdminClass) => {
  if (!isAdmin.value) return
...
const saveAdminClass = async () => {
  ...
  try {
    ...
    if (editingId.value) {
      await adminClassService.update(editingId.value, payload)
      uiStore.notify('Cập nhật lớp hành chính thành công', 'success')
    } else {
      await adminClassService.create(payload)
...
const openDeleteDialog = (c: AdminClass) => {
  if (!isAdmin.value) return
  deleting.value = c
...
const confirmDelete = async () => {
  ...
  try {
    await adminClassService.delete(deleting.value.id)
