...
public interface JpaCourseRepository extends JpaRepository<CourseEntity, Long> {
    ...
    @Query("SELECT c FROM CourseEntity c WHERE " +
           ...
           "(:active IS NULL OR c.active = :active) AND " +
           "(:status IS NULL OR c.status = :status)")
    Page<CourseEntity> filter(Long semesterId, Long subjectId, Long teacherId, Boolean active, CourseLifecycleStatus status, Pageable pageable);
    ...
    @Query("SELECT COUNT(c) FROM CourseEntity c WHERE " +
           ...
           "(:active IS NULL OR c.active = :active) AND " +
           "(:status IS NULL OR c.status = :status)")
    long countFilter(Long semesterId, Long subjectId, Long teacherId, Boolean active, CourseLifecycleStatus status);
}
